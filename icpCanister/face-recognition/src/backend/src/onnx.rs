...
thread_local! {
    ...
    static DEEPFAKE_DETECTION: RefCell<Option<Model>> = RefCell::new(None);
...
fn setup_deepfake(bytes: Bytes) -> TractResult<()> {
    let proto: tract_onnx::pb::ModelProto = tract_onnx::pb::ModelProto::decode(bytes)?;
    let deepfake = tract_onnx::onnx()
        .model_for_proto_model(&proto)?
        .into_optimized()?
        .into_runnable()?;
    DEEPFAKE_DETECTION.with_borrow_mut(|m| {
        *m = Some(deepfake);
    }); 
    Ok(())
}

pub fn setup(facedetect: Bytes, facerec: Bytes, deepfake: Bytes) -> TractResult<()> {
    setup_facedetect(facedetect)?;
    setup_facerec(facerec)?;
    setup_deepfake(deepfake)
}

// pub fn setup(deepfake: Bytes) -> TractResult<()> {
//     setup_deepfake(deepfake)?
// }
...
pub fn check_deepfake(image: Vec<u8>) -> Result<bool, anyhow::Error> {
    DEEPFAKE_DETECTION.with_borrow(|model| {
        let model = model.as_ref().unwrap();
        let image = image::load_from_memory(&image)?.to_rgb8();
        let image =
        ...
        let result = model.run(tvec!(Tensor::from(tensor).into()))?;
        let is_deepfake = result[0].to_array_view::<f32>()?.to_owned().into_raw_vec()[0];
        Ok(is_deepfake > 0.5)
    })
}
