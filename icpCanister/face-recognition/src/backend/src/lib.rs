...
const DEEPFAKE_DETECTION_FILE: &str = "ultralight_deepfake_detector.onnx";   
...
/// The result of the deepfake detection endpoint.
#[derive(CandidType, Deserialize)]
enum DeepfakeDetection {
    Ok(bool),
    Err(Error),
}
...
/// Checks if the given image is a deepfake.
#[ic_cdk::update]
fn check_deepfake(image: Vec<u8>) -> DeepfakeDetection {
    let result = match onnx::check_deepfake(image) {
        Ok(result) => DeepfakeDetection::Ok(result),
        Err(err) => DeepfakeDetection::Err(Error {
            message: err.to_string(),
        }),
    };
    result
...
/// Clears the deepfake detection model file.
/// This is used for incremental chunk uploading of large files.
#[ic_cdk::update]
fn clear_deepfake_detection_model_bytes() {
    storage::clear_bytes(DEEPFAKE_DETECTION_FILE);
}
...
/// Appends the given chunk to the deepfake detection model file.
/// This is used for incremental chunk uploading of large files.
#[ic_cdk::update]
fn append_deepfake_detection_model_bytes(bytes: Vec<u8>) {
    storage::append_bytes(DEEPFAKE_DETECTION_FILE, bytes);
}
...
fn setup_models() -> Result<(), String> {
    setup(
        ...
        storage::bytes(DEEPFAKE_DETECTION_FILE),
    )
